...
function xyzTable(P) {
  const rows = P.map((p, i) =>
    "<tr" + (i === 3 ? ' class="fw-bold"' : "") + "><td>" + (i + 1) + "</td>" +
    p.map(v => '<td class="text-end">' + v.toFixed(4) + "</td>").join("") + "</tr>"
  ).join("");

  return '<table class="table table-sm font-monospace small tabular mt-3 mb-0">' +
...
function checkBlock(P, r, theta, phi) {
  const line = (name, got, asked) =>
    "  " + name.padEnd(6) + "= " + got.toFixed(6).padStart(11) +
    "   (asked for " + asked.toFixed(6) + ")";

  return '<pre class="bg-body-tertiary border rounded p-3 small mt-3 mb-0">' +
...
function selfTest() {
  ...
  const check = (name, fn) => {
    ...
    catch (e) { results.push([name, String(e)]); }
  };
  ...
  check("round trip", () => {
    const cases = [
      ...
      [1.33,  1.50,  120.0, 1.10,  118.0,    0],
      [2.05,  1.21,  175.0, 1.44,   91.3, 137.8],
      [1.00,  1.00,   90.0, 1.00,   90.0, -179.9]
    ];
    let worst = 0;
  ...
  check("tetrahedral geometry", () => {
    const r = 1.087, tet = Math.acos(-1 / 3) * R2D, C = [0, 0, 0];
  ...
  check("rigid-body invariance", () => {
    const A = [0.11, -0.42, 0.77], B = [1.63, 0.05, -0.21], C = [2.02, 1.44, 0.63];
    const D = placeAtom(A, B, C, 1.526, 112.7, -73.4);

    const axis = [0.37, -0.81, 0.45], ang = 1.234, shift = [-3.1, 7.7, 0.9];
  ...
  check("atom 3 closed form", () => {
    const r12 = 1.526, r = 1.43, th = 109.5;
  ...
  const report = results
    .map(x => x[1] === true ? "PASS  " + x[0] : "FAIL  " + x[0] + " -- " + x[1])
    .join("\n");

  const badge = $("badge");
  badge.className = "badge rounded-pill font-monospace " +
